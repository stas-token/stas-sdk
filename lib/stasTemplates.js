...
class StasUtils {
    ...
    updateStasScript (destinationPublicKeyHash, script) {
        if (script.match(this.stasScriptOwnerUpdateRegex) === null) {
        throw new Error('Invalid STAS script')
        }
        return script.replace(this.stasScriptOwnerUpdateRegex, `$1${destinationPublicKeyHash}$3`)
        // If hitting error "Maximum call stack size exceeded" for 10mb + script data can use manual approach below
     ...
     * @returns {string} returns ASM string representation
     */
     ...
     * @param {array} data - data array containing string values
     * @returns {string} returns hex string representation
     */
     ...
     * @returns {string} returns hex string with data added to the script
     */
     ...
     * @returns {boolean} - true if the script matches the STAS-789 prefix
     */
     ...
     * @param {string} script - hex representation of the STAS script
     * @returns {boolean} - true if the script matches the STAS-20 prefix
     */
     ...
     * @param {string} script - hex representation of the STAS script
     * @returns {boolean} - true if the script matches the STAS-50 prefix
     */
    ...
    /**
     * Will return the meta data in the STAS script after the token symbol
     * @param {string} script - hex representation of the STAS script
     * @returns {(array|null)} - if the script is valid STAS type it will return an array of data or else null
     */
    getScriptData (script) {
        const scriptBuf = bsv.Script.fromHex(script).toASM()
        let res = scriptBuf.toString().match(this.stasFlagsDataRegex)
        if(!res){
            res = scriptBuf.toString().match(this.stasNoFlagsDataRegex)
            if(res && res.length > 1){
            let data = scriptBuf.split('OP_RETURN')[1]
            let dataSegments = data.split(' ')
            return dataSegments.slice(4)
            }}
        else
        if (res && res.length > 3) {
        return res.slice(3)
     ...
     * @param {string} script - hex representation of the STAS script
     * @returns {(string|null)} - if the script is valid STAS type that contains flags it will return flag value of 00 or 01 or else null
     */
    getScriptFlags (script) {
        const scriptBuf = bsv.Script.fromHex(script).toASM()
        const res = scriptBuf.toString().match(this.stasFlagsDataRegex)
        if (res && res.length > 1) {
            return res[1]
        }
        return null
     ...
     * @param {string} script - hex representation of the STAS script
     * @returns {string} publicKeyHash - represents the redeem address of the token script
     */
    getRedeemPublicKeyHash (script) {
        const scriptBuf = bsv.Script.fromHex(script).toASM()
        const split = scriptBuf.split('OP_RETURN')[1]
     ...
     * @param {string} script - hex representation of the STAS script
     * @returns {(string|null)} will return the token symbol or null
     */
    getSymbol (script) {
        const scriptBuf = bsv.Script.fromHex(script).toASM()
        let res = scriptBuf.toString().match(this.stasFlagsDataRegex)
     ...
     * @param {string} script - hex representation of the STAS script
     * @returns {boolean} - will return return if STAS script is of splittable type
     */
    isSplittable (script) {
        let isSplittable 
        const scriptBuf = bsv.Script.fromHex(script).toASM()
     ...
     * @param {string} script - hex representation of the STAS script
     * @returns {boolean} - true if the script is of an existing STAS script used in this library
     */
    isStasScript (script) {
     ...
     * @returns {string} - will return token symbol string where present
     */
     ...
     * @returns {string} - will return the STAS token script in ASM format
     */
     ...
     * @returns {string} - will return the signed unlocking script in ASM format
     */
    buildUnlockingScript (tx, outputs, sigString, pubKeyString, isStas789, isStas20, data, isZeroFee, isZeroChange) {
        ...
        for(let i = 0; i < outputs.length; i++){
            const output = outputs[i]
                unlockScript += `${this.utility.numberToLESM(output.satoshis)} ${Buffer.from(output.publicKeyHash)} `
                if(isStas789 && i === 0){
        ...
        if(isZeroFee) {
            ...
            const reversedFundingTXID = this.utility.reverseEndian(tx.inputs[tx.inputs.length - 1].prevTxId)
            unlockScript += `${this.utility.numberToLESM(outpointFundingIndex)} ${Buffer.from(reversedFundingTXID)}`              
        }
     ...
     * @param {boolean} isZeroFee - will determine if the unlocking script will contain a fee input and a change output 
     * @param {boolean} isZeroChange - will determine if the unlocking script will contain a change output 
     * @returns {string} - will return the unsigned unlocking script in ASM format
     */
    ...
    */
      async buildMergeUnlockingScript(tx, stasUtxo, outputData, preImage, paymentUtxo, isStas20, data, isZeroChange, isZeroFee) {
        ...
        if(isZeroFee) {
            ...
            if(isZeroChange){
                customScript += `${this.utility.numberToLESM(paymentUtxo.vout)} ${this.utility.reverseEndian(paymentUtxo.txid)} `
            } 
        }

        let outputScript = ``
        for(let i = 0; i < outputData.length; i++){
                outputScript += `${this.utility.numberToLESM(tx.outputs[i].satoshis)} ${outputData[i].publicKeyHash} `
        }
    
        let unlockingScript = ``
        if (isZeroFee || isZeroChange) {
            unlockingScript += `${outputScript}${customScript}${this.utility.numberToLESM(stasUtxo.vout)} ${stasUtxo.scriptPiece} ${this.utility.numberToLESM(stasUtxo.pieceCount)} ${preImage}`
          
        } else {
            unlockingScript += `${outputScript}${scriptData}${this.utility.numberToLESM(paymentUtxo.vout)} ${this.utility.reverseEndian(paymentUtxo.txid)} ${this.utility.numberToLESM(stasUtxo.vout)} ${stasUtxo.scriptPiece} ${this.utility.numberToLESM(stasUtxo.pieceCount)} ${preImage}`
        }
