...
class Utility {
    constructor() {
      this.SATS = 1; // Don't change this setting unless you know what you're doing
      ...
      this.P2PKH_LOCKING_SCRIPT_BYTES = 22
      this.FLAGS_BYTES = 8
  ...
  */
  addressToP2pkh(addr) {
    ...
    return bsv.Script.buildPublicKeyHashOut(addr).toHex();
  }

    /**
    * will return address from bsv privateKey
    * @param {string} privateKey - is bsv privateKey
    * @return {string} address
  */
  privKeyToAddressStr(privateKey) {
    if (typeof privateKey === 'string') {
      privateKey = bsv.PrivateKey.fromString(privateKey);
    }
    return bsv.Address.fromPrivateKey(privateKey).toString();
  }
    ...
    replaceAndSplitWithLimit(str, searchValue, replaceValue, limit) {
      // First, replace all occurrences of searchValue with replaceValue
      const replaced = str.split(searchValue).join(replaceValue);
      
      // Then, split the result, but limit the number of splits
      const parts = replaced.split(replaceValue, limit);
      
      // If we haven't reached the limit, and there's remaining string,
      // add it as the last element
      if (parts.length < limit) {
        const lastPart = replaced.slice(parts.join(replaceValue).length + replaceValue.length);
        if (lastPart) {
          //parts.push(lastPart);
        }
      }
